...
class PaymentController {
  ...
  static async createPayment(req, res, next) {
    try {
      ...
      // 验证工单
      const order = await WorkOrder.findById(order_id);
      if (!order) {
      ...
      // 验证报价
      if (!order.quoted_price || order.quoted_price <= 0) {
  ...
  static async confirmTestPayment(req, res, next) {
    try {
      ...
      await Payment.handlePaymentSuccess(payment_no, transactionData);
  ...
  static async wechatNotify(req, res, next) {
    try {
      ...
      if (verifyResult.success) {
        // 处理支付成功
        await Payment.handlePaymentSuccess(verifyResult.payment_no, {
          transaction_id: verifyResult.transaction_id,
          total_fee: verifyResult.total_fee,
          time_end: verifyResult.time_end,
          openid: verifyResult.openid
        });
      ...
      if (payment.status === 'pending' && payment.payment_method === 'wechat') {
        try {
          ...
          if (queryResult.success && queryResult.trade_state === 'SUCCESS') {
            // 支付成功，更新状态
            await Payment.handlePaymentSuccess(payment_no, {
              transaction_id: queryResult.transaction_id,
              total_fee: queryResult.total_fee
            });
            
            // 重新查询更新后的支付记录
            const updatedPayment = await Payment.findByPaymentNo(payment_no);
